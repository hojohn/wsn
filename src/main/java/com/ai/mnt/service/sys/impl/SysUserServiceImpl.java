...
public class SysUserServiceImpl implements SysUserService {
    ...
    private void cvtContentList(List<SysUser> sysUserList) {
        for(SysUser sysUser : sysUserList) {
           if (sysUser.getBaseId() == 1) {
            sysUser.setBaseIdTxt("管理人员");
        }else {
            sysUser.setBaseIdTxt("普通用户");
        }
          if (sysUser.getUserSts().equals("1") ) {
            sysUser.setUserStsTxt("正常");
        }else {
            sysUser.setUserStsTxt("锁定");
        }
