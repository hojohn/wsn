...
<script>
    $(function() {
        ...
        $('.reload').on('click', function() {
            $('#username').val('');
            $('#password').val('');
            tableObj.ajax.url('user/info/query?username=-1').load();
        });
        ...
        $('.div_right').on('click', '.dt-buttons>#btnUpdate',function() {
            ...
            var strUsername = checkedBox.find('input:checkbox').val();
            showModal(2, " ", [ '1000px', '500px' ], "user/info/update_page/" + strUsername, tableObj);
        });
        
        //新增
        $('.div_right').on('click', '.dt-buttons>#btnAdd', function(){
            showModal(2, " ", ['1000px', '500px'], "user/info/add_page", tableObj);
        });
        
        //删除按钮
        $('.div_right').on('click', '.dt-buttons>#btnDelete',function() {
            var checkedBox = $("#tabUser tbody tr td .checked");
            comfirmDeleteData(tableObj, checkedBox, 'user/info/delete/');
        });
    ...
    function getConfig() {
        return config = {
            ...
            "serverSide" : false,
            "ajax" : {
                "url" : "user/info/query?username=-1"
            //    "async": false
            },
            "columns" : [ {
                "data" : "username"
