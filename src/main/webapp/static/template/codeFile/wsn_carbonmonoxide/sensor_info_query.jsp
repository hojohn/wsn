                ...
                <form action="#" class="form-horizontal">
                    <div class="form-body">
                        <div class="row">
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label class="control-label col-md-4">传感器编号</label>
                                    <div class="col-md-8">
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label class="control-label col-md-4">传感器一氧化碳值</label>
                                    <div class="col-md-8">
                                        <input type="text" class="form-control" id="carbonmonoxideValue">
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label class="control-label col-md-4">状态：0正常，1损坏，2丢失</label>
                                    <div class="col-md-8">
...
<!--                     </div> -->
                    <div class="portlet-body">
                        <table class="table table-striped table-bordered table-hover table-checkable order-column" id="tabWsnCarbonmonoxide" cellspacing="0" width="100%">
                            <thead>
                                <tr>
...
<script>
    $(function() {
        ...
        var tableObj = $('#tabWsnCarbonmonoxide').DataTable(getConfig());
        $('#btnQuery').click(function(e) {
            var strWareId = $("#wareId").val();
            var strObjId = $("#objId").val();
            var strCarbonmonoxideValue = $("#carbonmonoxideValue").val();
            var strRate = $("#rate").val();
            var strState = $("#state").val();
            var strCreateTime = $("#createTime").val();
            var strField = $("#field").val();
            var strId = $("#id").val();
            var sUrl = "sensor/info/query?";
            sUrl += 
                "&wareId=" + strWareId +
                "&objId=" + strObjId +
                "&carbonmonoxideValue=" + strCarbonmonoxideValue +
                "&rate=" + strRate +
                "&state=" + strState +
        ...
        $('.reload').on('click', function() {
            $('#wareId').val('');
            $('#objId').val('');
            $('#carbonmonoxideValue').val('');
            $('#rate').val('');
            $('#state').val('');
            $('#createTime').val('');
            $('#field').val('');
            $('#id').val('');
            tableObj.ajax.url('sensor/info/query?id=-1').load();
        });
        
        //修改按钮
        $('.div_right').on('click', '.dt-buttons>#btnUpdate',function() {
            var checkedBox = $("#tabWsnCarbonmonoxide tbody tr td .checked");
            ...
            showModal(2, " ", [ '1000px', '500px' ], "sensor/info/update_page/" + strId, tableObj);
        });
        
        //新增
        $('.div_right').on('click', '.dt-buttons>#btnAdd', function(){
            showModal(2, " ", ['1000px', '500px'], "sensor/info/add_page", tableObj);
        });
        
        //删除按钮
        $('.div_right').on('click', '.dt-buttons>#btnDelete',function() {
            var checkedBox = $("#tabWsnCarbonmonoxide tbody tr td .checked");
            comfirmDeleteData(tableObj, checkedBox, 'sensor/info/delete/');
        });
