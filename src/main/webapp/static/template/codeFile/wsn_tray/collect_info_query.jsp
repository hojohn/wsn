                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <div class="col-md-8">
                                        <input type="text" class="form-control" id="wareTime">
                                    </div>
                                </div>
                            </div>
                        </div>
...
<!--                     </div> -->
                    <div class="portlet-body">
                        <table class="table table-striped table-bordered table-hover table-checkable order-column" id="tabWsnTray" cellspacing="0" width="100%">
                            <thead>
                                <tr>
...
<script>
    $(function() {
        ...
        var tableObj = $('#tabWsnTray').DataTable(getConfig());
        $('#btnQuery').click(function(e) {
            var strWareId = $("#wareId").val();
            var strWareTime = $("#wareTime").val();
            var sUrl = "collect/info/query?";
            sUrl += 
                "&wareId=" + strWareId +
                "&wareTime=" + strWareTime;
        ...
        $('.reload').on('click', function() {
            $('#wareId').val('');
            $('#wareTime').val('');
            tableObj.ajax.url('collect/info/query?wareId=-1').load();
        });
        
        //修改按钮
        $('.div_right').on('click', '.dt-buttons>#btnUpdate',function() {
            var checkedBox = $("#tabWsnTray tbody tr td .checked");
            ...
            showModal(2, " ", [ '1000px', '500px' ], "collect/info/update_page/" + strWareId, tableObj);
        });
        
        //新增
        $('.div_right').on('click', '.dt-buttons>#btnAdd', function(){
            showModal(2, " ", ['1000px', '500px'], "collect/info/add_page", tableObj);
        });
        
        //删除按钮
        $('.div_right').on('click', '.dt-buttons>#btnDelete',function() {
            var checkedBox = $("#tabWsnTray tbody tr td .checked");
            comfirmDeleteData(tableObj, checkedBox, 'collect/info/delete/');
        });
    ...
    function getConfig() {
        return config = {
            ...
            "serverSide" : false,
            "ajax" : {
                "url" : "collect/info/query?wareId=-1"
            //    "async": false
            },
            "columns" : [ {
                "data" : "wareId"
