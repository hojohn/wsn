                        ...
                        <div class="row">
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label class="control-label col-md-4">硬件类型ID</label>
                                    <div class="col-md-8">
...
<!--                     </div> -->
                    <div class="portlet-body">
                        <table class="table table-striped table-bordered table-hover table-checkable order-column" id="tabWsnHardwareType" cellspacing="0" width="100%">
                            <thead>
                                <tr>
...
<script>
    $(function() {
        ...
        var tableObj = $('#tabWsnHardwareType').DataTable(getConfig());
        $('#btnQuery').click(function(e) {
            var strTypeId = $("#typeId").val();
            var strTypeName = $("#typeName").val();
            var strRemark = $("#remark").val();
            var sUrl = "device/type/query?";
            sUrl += 
                "&typeId=" + strTypeId +
        ...
        $('.reload').on('click', function() {
            ...
            $('#typeName').val('');
            $('#remark').val('');
            tableObj.ajax.url('device/type/query?typeId=-1').load();
        });
        
        //修改按钮
        $('.div_right').on('click', '.dt-buttons>#btnUpdate',function() {
            var checkedBox = $("#tabWsnHardwareType tbody tr td .checked");
            ...
            var strTypeId = checkedBox.find('input:checkbox').val();
            showModal(2, " ", [ '1000px', '500px' ], "device/type/update_page/" + strTypeId, tableObj);
        });
        
        //新增
        $('.div_right').on('click', '.dt-buttons>#btnAdd', function(){
            showModal(2, " ", ['1000px', '500px'], "device/type/add_page", tableObj);
        });
        
        //删除按钮
        $('.div_right').on('click', '.dt-buttons>#btnDelete',function() {
            var checkedBox = $("#tabWsnHardwareType tbody tr td .checked");
            comfirmDeleteData(tableObj, checkedBox, 'device/type/delete/');
        });
    ...
    function getConfig() {
        return config = {
            ...
            "serverSide" : false,
            "ajax" : {
                "url" : "device/type/query?typeId=-1"
            //    "async": false
            },
            "columns" : [ {
                "data" : "typeId"
